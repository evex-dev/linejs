...
import { TalkClient } from "../internal/talk-client";
import { LooseType } from "../../entities/common";
import { RawReadStruct as readStruct } from "../../libs/thrift/read";
import { Message } from "../../libs/thrift/line_types";

const Thrift = thrift.Thrift;

class E2EE extends TalkClient {
    async getE2EESelfKeyData(mid: string) {
        try {
    ...
    getE2EESelfKeyDataByKeyId(keyId: string | number) {
        try {
            return JSON.parse(this.storage.get("e2eeKeys:" + keyId) as string);
        } catch { }
    }
    saveE2EESelfKeyDataByKeyId(keyId: string | number, value: LooseType) {
    ...
    async getE2EELocalPublicKey(mid: string, keyId: string | number | undefined) {
        ...
        if (toType === 0) {
            ...
            if (!key) {
                let E2EEGroupSharedKey = await this.getLastE2EEGroupSharedKey({ keyVersion: 2, chatMid: mid });
                ...
                const selfKey = Buffer.from(
                    this.getE2EESelfKeyDataByKeyId(receiverKeyId)["privKey"], "base64"
                );
    ...
    generateSharedSecret(privateKey: Buffer, publicKey: Buffer) {
    ...
    getSHA256Sum(...args: string[] | Buffer[]) {
    ...
    _encryptAESECB(aesKey: LooseType, plainData: LooseType) {
    ...
    decryptKeyChain(publicKey: Buffer, privateKey: Buffer, encryptedKeyChain: Buffer) {
        ...
        const binkey = bin2bytes(keychainData.toString("hex"));
        const key = readStruct(binkey)[1]
        const publicKeyBytes = Buffer.from(key[0][4]);
        const privateKeyBytes = Buffer.from(key[0][5]);
        return [privateKeyBytes, publicKeyBytes];
    }

    encryptDeviceSecret(publicKey: Buffer, privateKey: Buffer, encryptedKeyChain: Buffer) {
        const sharedSecret = this.generateSharedSecret(privateKey, publicKey);
        const aesKey = this.getSHA256Sum(sharedSecret, "Key");
    ...
    generateAAD(a: string, b: string, c: number, d: number, e = 2, f = 0) {
    ...
    encryptE2EEMessageV2(data: Buffer, gcmKey: Buffer, nonce: Buffer, aad: Buffer) {
    ...
    async decryptE2EETextMessage(messageObj: Message, isSelf = true) {
    ...
    async decryptE2EELocationMessage(messageObj: Message, isSelf = true) {
        const _from = messageObj._from;
        const to = messageObj.to;
        const toType = messageObj.toType;
        const metadata = messageObj.contentMetadata;
    ...
    decryptE2EEMessageV1(chunks:Buffer[], privK:Buffer, pubK:Buffer) {
        const salt = chunks[0];
