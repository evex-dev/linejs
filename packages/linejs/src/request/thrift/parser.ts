...
export default class ThriftRenameParser {
	...
	rename_data(data: ParsedThrift): any {
	...
	parse_data(structName: string, object: any): any[][] {
		const newThrift = [];
		for (const fname in object) {
			...
			if (finfo.struct) {
				...
				if (typeof finfo.list === "number") {
					...
					thisValue[2] = [
						TYPE.STRUCT,
						value.map((e: any) => this.parse_data(finfo.list, e)),
					];
				}
			} else if (finfo.map) {
				...
				if (typeof finfo.set === "number") {
					...
					thisValue[2] = [
						TYPE.STRUCT,
						value.map((e: any) => this.parse_data(finfo.set, e)),
					];
				}
			} else if (finfo.type) {
