		...
		res.events.forEach(async (e) => {
			...
			switch (e.type) {
				case 29:
					...
					let date = e.payload.notificationMessage.squareMessage.message
						.deliveredTime;
					date = new Date(date);
					date = date.getMonth() + 1 + "/" + date.getDate();
					chat.timeInt = e.payload.notificationMessage.squareMessage.message
						.deliveredTime;
					if (
						e.payload.notificationMessage.squareMessage.message.text
					) {
						chat.lastText = e.payload.notificationMessage.squareMessage.message
							.text;
					}
							...
							svg(
								...
								g(
									...
									path(
										{
											"d":
												"M3.75 4c0-.69.56-1.25 1.25-1.25h13c.69 0 1.25.56 1.25 1.25v14.004c0 .69-.56 1.25-1.25 1.25H5c-.69 0-1.25-.56-1.25-1.25V4zM8.523 8.026h5.954M8.523 10.979h5.954M8.523 13.979h5.954",
										},
									),
								),
								defs(
					...
					svg(
						...
						g(
							...
							path(
								{
									"d":
										"m13 3.6245-.0002 13.046 5.7122-5.4163 1.3762 1.4512L12 20.3754l-8.0882-7.67 1.3762-1.4512 5.7118 5.4163L11 3.6245h2Z",
								},
							),
						),
					),
					...
					i(
						...
						svg(
							...
							path(
								{
									"d":
										"M14.843 13.17a.624.624 0 0 0-.853.228 2.76 2.76 0 0 1-4.78 0 .624.624 0 1 0-1.08.625 4.008 4.008 0 0 0 6.94 0 .624.624 0 0 0-.227-.852z",
									"fill": "#303030",
								},
							),
							circle(
							...
							path(
								{
									...
									"clip-rule": "evenodd",
									"d":
										"M11.6 3.22a8.88 8.88 0 1 0 8.88 8.88 8.89 8.89 0 0 0-8.88-8.88zm0 16.512a7.632 7.632 0 1 1 7.632-7.632 7.64 7.64 0 0 1-7.632 7.632z",
									"fill": "#303030",
								},
							),
							path(
						...
						svg(
							...
							g(
								...
								path(
									{
										"d":
											"m20.1864 11.0044-15.477-7.736a1.1091 1.1091 0 0 0-1.165.105c-.338.254-.503.671-.427 1.087l1.274 7.04h8.108v1h-8.119l-1.263 7.039c-.076.417.089.833.427 1.087.198.148.432.224.667.224.169 0 .34-.039.499-.118l15.476-7.737c.379-.19.614-.571.614-.995 0-.425-.235-.806-.614-.996Z",
									},
								),
							),
						),
...
function msgMain(data, raw) {
							...
							svg(
								...
								g(
									...
									path(
										{
											"d":
												"M18.105 11.437a.665.665 0 0 1 0 1.126L7.412 19.29a.665.665 0 0 1-1.02-.563V5.274a.665.665 0 0 1 1.02-.563l10.693 6.726z",
										},
									),
								),
							),
	...
	function flexMsg(data) {
		...
		let html =
			'<!doctype html><html class="in_iframe"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1, maximum-scale=1"><meta name="format-detection" content="telephone=no"><link rel="stylesheet" type="text/css" href="https://pamornt.github.io/flex2html/css/flex2html.css"></head><body style="margin: 0;"><div>';
		html += flex2html("hide", {
			"type": "flex",
