...
client.saveE2EESelfKeyDataByKeyId("{keyId}", {
	"keyId": "",
	"privKey": "++fVY=",
	"pubKey": "~=",
	"e2eeVersion": "1",
});
client.saveStorageData(
	"e2eeGroupKeys:{c}",
	'{"privKey": "+di7+=", "keyId": 000}',
);
const e = await client.sync({ revision: N });
console.log(e.operationResponse.operations[0].message);
console.log("decrypt e2ee txt");
console.log(
	await client.decryptE2EETextMessage(
		e.operationResponse.operations[0].message,
	),
);
```
