...
class E2EE {
    getE2EELocalPublicKey(mid, keyId) {
        ...
        if (toType === 0) {
            ...
            if (key === null) {
                ...
                const specVersion = this.checkAndGetValue(receiverKeyData, "specVersion", 3);
                if (specVersion === -1) {
                ...
                if (receiverKeyId === keyId) {
                    ...
                    throw new Error(`E2EE key id-${keyId} not found on ${mid}, key id should be ${receiverKeyId}`);
                }
            }
        } else {
            fd = ".e2eeGroupKeys";
            ...
            if (key === null) {
                ...
                const groupKeyId = this.checkAndGetValue(E2EEGroupSharedKey, "groupKeyId", 2);
                const creator = this.checkAndGetValue(E2EEGroupSharedKey, "creator", 3);
                const creatorKeyId = this.checkAndGetValue(E2EEGroupSharedKey, "creatorKeyId", 4);
                const receiver = this.checkAndGetValue(E2EEGroupSharedKey, "receiver", 5);
                const receiverKeyId = this.checkAndGetValue(E2EEGroupSharedKey, "receiverKeyId", 6);
                const encryptedSharedKey = this.checkAndGetValue(E2EEGroupSharedKey, "encryptedSharedKey", 7);
                const selfKey = atob(this.getE2EESelfKeyDataByKeyId(receiverKeyId)["privKey"]);
                ...
                const aes = CryptoJS.AES.decrypt({ ciphertext: encryptedSharedKey }, aes_key, {
                    iv: aes_iv,
    ...
    generateSharedSecret(privateKey, publicKey) {
        return curve25519.sharedKey(Uint8Array.from(privateKey), Uint8Array.from(publicKey));
    }
    ...
    decryptKeyChain(publicKey, privateKey, encryptedKeyChain) {
        ...
        let keychainData = Buffer.concat([decipher.update(encryptedKeyChain), decipher.final()]);
    ...
    encryptE2EEMessage(to, text, specVersion = 2, isCompact = false, contentType = 0) {
        ...
        if (this.getToType(to) === 0) {
            ...
            const receiverKeyDataBuffer = this.checkAndGetValue(publicKey, "keyData", 4);
    ...
    encryptE2EETextMessage(
        ...
        this.log(`senderKeyId: ${senderKeyId} (${bSenderKeyId.toString('hex')})`, true);
        this.log(`receiverKeyId: ${receiverKeyId} (${bReceiverKeyId.toString('hex')})`, true);
    ...
    decryptE2EETextMessage(messageObj, isSelf = true) {
        ...
        chunks = chunks.map(chunk => typeof chunk === 'string' ? Buffer.from(chunk, 'utf-8') : chunk);
        ...
        let decrypted;
        if (specVersion === "2") {
            decrypted = this.decryptE2EEMessageV2(to, _from, chunks, privK, pubK, specVersion, contentType);
        } else {
