import * as LINETypes from "../../packages/types/line_types.ts"
import { parseEnum } from "../../packages/types/thrift.ts"
import { Client } from "../../packages/linejs/client/index.ts"
...
export class Message {
    public sourceType: 0 | 1 | 2 | 3;// op noti recv send
    public rawSource: LINETypes.Operation | LINETypes.SquareEventNotificationMessage | LINETypes.SquareEventReceiveMessage | LINETypes.SquareEventSendMessage;
    ...
    constructor(options: {
        ...
        const { operation, squareEventNotificationMessage, squareEventReceiveMessage, squareEventSendMessage } = options;
        if (operation && ((operation.type === "SEND_MESSAGE" || operation.type === 25) || (operation.type === "RECEIVE_MESSAGE" || operation.type === 26) || (operation.type === "SEND_CONTENT" || operation.type === 43))) {
            ...
            this._senderDisplayName = squareEventNotificationMessage.senderDisplayName
            this.sourceType = 1
        ...
        this.toType = parseEnum("MIDType", this.rawMessage.toType) as LINETypes.MIDType || this.rawMessage.toType
    ...
    public getEmojis(): string[] {
        ...
        emojiData.REPLACE.sticon.resources.forEach(e => {
            emojiUrls.push(`https://stickershop.line-scdn.net/sticonshop/v1/sticon/${e.productId}/android/${e.sticonId}.png`)
        })
        return emojiUrls
    }
    ...
    public getFlex() {
        ...
        return { flexJson: flexData.FLEX_JSON, altText: flexData.ALT_TEXT, ver: flexData.FLEX_VER, tag: flexData.EFFECT_TAG }
    }
    ...
    public getReply(): string | undefined {
        if (this.rawMessage.relatedMessageId && (this.rawMessage.messageRelationType === 3 || this.rawMessage.messageRelationType === "REPLY")) {
...
export class ClientMessage extends Message {
     ...
     */
    public getData(preview?: boolean) {
...
export class TalkMessage extends ClientMessage {
    constructor(operation: LINETypes.Operation, client: Client) {
    ...
    public getGroup() {
        if (this.toType === "GROUP" || this.toType === "ROOM") {
            return this.client.getChats({ gids: [this.to] }).then(e => { return e.chats[0] as LINETypes.Chat })
        }
    }
...
export class SquareMessage extends ClientMessage {
    ...
    public async getSquare() {
        return await this.client.getSquare((await this.getSquareChat()).squareChat.squareMid)
    }
